            ...
            <table class="w-full whitespace-no-wrap border table-auto">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    @foreach($dataPty as $key => $dp)
                        ...
                        @if(isset(($dataPty[$key + 1])))
                            @php 
                                $nextRow = $dataPty[$key + 1]
                            @endphp

                            @if($dp->namasite != $nextRow->namasite)
                                <tr class="data-row text-center text-gray-700 bg-gray-500 dark:text-gray-400 hover:bg-gray-400 hover:text-white ease-in-out duration-150">
                                    @for($i=0; $i<19; $i++)
                                        <td class="px-4 py-3 text-sm"></td>
                                    @endfor
                                </tr>
                            @endif
                        @elseif($key == count($dataPty)-1)
                            <tr class="data-row text-center text-gray-700 bg-gray-500 dark:text-gray-400 hover:bg-gray-400 hover:text-white ease-in-out duration-150">
                                @for($i=0; $i<19; $i++)
                                    <td class="px-4 py-3 text-sm"></td>
                                @endfor
